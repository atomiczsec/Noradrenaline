...
assert_contains "$empty_output" "Meaning: Stores current-user privacy decisions"
assert_contains "$empty_output" "Active exceptions:"
...
assert_contains "$populated_output" "Type: regular file"
assert_contains "$populated_output" "Size: 0 bytes"
assert_contains "$populated_output" "Owner UID:"
assert_contains "$populated_output" "Mode:"
assert_contains "$populated_output" "Modified:"
...
assert_contains "$populated_output" "does not prove FileVault is enabled"
assert_contains "$populated_output" "[+] Indicator: Screen Time preferences"
assert_contains "$populated_output" "Evidence collected: file metadata and in-process SIP state"
